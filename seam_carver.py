...
SAMPLE_IMAGE_PATH = "RenderedImage.jpeg"
...
def compute_energy(image: np.ndarray):
    ...
    #create a 2D array the size of the image, and fill it with zeros
    energy = np.zeros( (height, width) ) 

    #traverse the energy array
    for h in range(height):
        for w in range(width):
            #if we are at the edge of the array 
            if h == 0 or h == height-1 or w == 0 or w == width-1: #pixel is on the edge
                #set the energy to 1000
                energy[h][w] = 1000
            else: #our pixel is in the center
                #get the 4 pixels around it
                up = image[h+1][w]
                down = image[h-1][w]
                left = image[h][w-1]
                right = image[h][w+1]
                changeX = 0
                changeY = 0
                #calculate the changeX + changeY for each color, and then add the sqaure to the overall
                for c in range(colors):
                    changeX += (right[c] - left[c]) ** 2
                    changeY += (down[c] - up[c]) ** 2
                #set energy value of middle pixel to sqaure root of sums of the changes
                energy[h][w] = (changeX + changeY) ** (1/2)
...
def find_vertical_seam(image: np.ndarray, energy=None):
   ...
   #create table of size height x width, but set each value to an array with 3 elements.
    table = []
    for h in range(height):
        arr = []
        for w in range(width):
        ...
        table += [arr]
    #INITALIZE:
    #initialize the top row with their energy values, w, and parentWidth(w)
    for w in range(width):
        table[0][w] = [energy[0][w], w, w]
    #FILL IN: 
    #fill in the table adding the current cell energy value with the smallest value in the row above.
    for h in range(1, height):
        for w in range(width):
            #if we are on the left edge, then only grab the top + topRight cells
            if w == 0:
                top = table[h-1][w]
                topRight = table[h-1][w+1]
                #find min of the above cells
                minCell = min(top, topRight)
            #if we are in the right edge, then grab the top + topLeft cells
            elif w == width-1:
                topLeft = table[h-1][w-1]
                top = table[h-1][w]
                #find min of the above cells
                minCell = min(topLeft, top)
            #if we are in the middle, then grab the top + topRight + topLeft cells
            else:
                topLeft = table[h-1][w-1]
                top = table[h-1][w]
                topRight = table[h-1][w+1]
                #find min of the above cells
                minCell = min(topLeft, top, topRight)
            #new value is the [energy + energy of minParent, w, widthOfParent]
            table[h][w] = [energy[h][w] + minCell[0], w, minCell[1]]
    #SOLUTION:
    solution = [math.inf, math.inf, math.inf]
    #find the minimum energy value on the bottom row of the table.
    for w in range(width):
        solution = min(solution, table[height-1][w])
    #BACKTRACK: 
    #append the current width of the solution to the coordinates array, and then reset the solution to
    #the height above and the width of the parent pixel.
    coordinates = []
    for h in range(height-1, -1, -1):
        coordinates += [solution[1]]
        solution = table[h-1][solution[2]]
